...
import { Row, Space, Button, Col, Input, Table, Modal, Form, Dropdown, Menu, Popconfirm } from 'antd';
import Icon from '@components/icon';
import { addIcon, refreshIcon, showIcon, exportIcon, syncOutlined } from '@/constant/icon.js';
...
export default function CodeCollectionContent(props) {
  ...
  const getInitData = () => {
    ...
    getCodeTableNameOpt().then(res => {
      ...
      if (state) {
        const newData = getTreeSelectData(data, 'name', 'id', true, 'comment', 'id');
  ...
  const getCodeTableNameSelectData = (arr = []) => {
    return arr.map(item => {
      if (item.children) {
        return {
          label: item.name,
          options: item.children.length ? getCodeTableNameSelectData(item.children) : [],
        };
      } else {
        return {
  ...
  const handleCodeTableNameSelect = (value, label, flag) => {
    flag === 'codeTableName' ? form.setFieldsValue({ ...deF }) : form.setFieldsValue({ ...deSF });
  ...
  const onFieldChange = (value, field, flag) => {
    if (value) {
      flag === 'field' ? (fieldValue[field] = value) : (fieldSubValue[field] = value);
    }
  ...
  const deleteFieldOpt = (value, field, flag) => {
    ...
    if (index >= 0) {
      ...
      flag === 'field' ? setFiledOpt(newFiledOpt) : setFiledItemOpt(newFiledOpt);
    }
  };
  ...
  const verify = () => {
    verifyCodeConfigTableExist({ codeConfigId: selectedRows[0]?.id }).then(res => {
      if (res.data.state) {
  ...
  const createTable = () => {
    codeSyncCreateTable({ codeSyncConfigId: selectedRows[0]?.id }).then(res => {
      const { state, msg } = res.data;
      if (state) {
        //建表成功调同步接口
        showInfo(msg);
  ...
  const type = currentType === 'customConfig' || currentType === 'defaultConfig' ? 'input' : 'treeSelect';
  const typeField = currentType === 'customConfig' || currentType === 'defaultConfig' ? 'input' : 'select';
  const hasChange = currentType === 'customConfig' || currentType === 'defaultConfig' ? false : true;
  const dis = currentType === 'customConfig' || currentType === 'informationSetConfig' ? false : true;
    ...
    data: [
      ...
      },
      {
        ...
        [type]: {
          ...
          disabled: dis,
          onChange: hasChange
            ? (value, label, extra) => handleCodeTableNameSelect(value, label, 'codeTableName')
            : undefined,
        },
      },
      {
        ...
        [type]: {
          ...
          disabled: dis,
          onChange: hasChange
            ? (value, label) => handleCodeTableNameSelect(value, label, 'codeItemTableName')
            : undefined,
        },
      },
      {
        ...
        [typeField]: {
          ...
          options: filedOpt,
          onChange: hasChange ? value => onFieldChange(value, 'codeNum', 'field') : undefined,
          disabled: dis,
        },
      },
      {
        ...
        [typeField]: {
          ...
          disabled: dis,
          onChange: hasChange ? value => onFieldChange(value, 'codeItemName', 'fieldSub') : undefined,
        },
      },
      {
        opts: {
        ...
        [typeField]: {
          ...
          options: filedOpt,
          disabled: dis,
          onChange: hasChange ? value => onFieldChange(value, 'codeName', 'field') : undefined,
        },
      },
      {
        opts: {
        ...
        [typeField]: {
          ...
          disabled: dis,
          options: filedItemOpt,
          onChange: hasChange ? value => onFieldChange(value, 'codeItemValue', 'fieldSub') : undefined,
        },
      },
      {
        opts: {
        ...
        [typeField]: {
          ...
          disabled: dis,
          options: filedOpt,
          onChange: hasChange ? value => onFieldChange(value, 'codeComment', 'field') : undefined,
        },
      },
      {
        opts: {
        ...
        [typeField]: {
          ...
          options: filedItemOpt,
          disabled: dis,
          onChange: hasChange ? value => onFieldChange(value, 'codeChildNodeCount', 'fieldSub') : undefined,
        },
      },
      {
        opts: {
        ...
        [typeField]: {
          ...
          options: filedOpt,
          disabled: dis,
          onChange: hasChange ? value => onFieldChange(value, 'codeSort', 'field') : undefined,
        },
      },
      {
        opts: {
        ...
        [typeField]: {
          ...
          options: filedItemOpt,
          disabled: dis,
          onChange: hasChange ? value => onFieldChange(value, 'itemSort', 'fieldSub') : undefined,
        },
      },
      {
        opts: {
        ...
        [typeField]: {
          ...
          disabled: dis,
          options: filedOpt,
          onChange: hasChange ? value => onFieldChange(value, 'classifyId', 'field') : undefined,
        },
      },
      {
        opts: {
        ...
        [typeField]: {
          ...
          disabled: dis,
          options: filedItemOpt,
          onChange: hasChange ? value => onFieldChange(value, 'enableName', 'fieldSub') : undefined,
        },
      },
      {
        opts: {
        ...
        [typeField]: {
          ...
          disabled: dis,
          options: filedOpt,
          onChange: hasChange ? value => onFieldChange(value, 'parentId', 'field') : undefined,
        },
      },
      {
        opts: {
          ...
          <Col span={17}>
            <Space>
              ...
              {/* <Popconfirm
                ...
                cancelText="否"
              ></Popconfirm> */}
              <Button type="primary" icon={<Icon type={syncOutlined} />} onClick={handleSynchronization}>
                同步
              </Button>
