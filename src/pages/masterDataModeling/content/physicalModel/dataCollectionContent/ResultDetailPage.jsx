...
import { Tabs, Table, Typography, Tooltip, Tag } from 'antd';
  ...
  return (
    <div style={{ marginLeft: 15 }}>
      ...
      {contentFlag === 'syncErrorList' || contentFlag === 'syncSuccessList' ? (
        ...
        <div>
          {record.primaryKeyErrorMessage && record.primaryKeyErrorMessage !== 'null'
            ? record.primaryKeyErrorMessage
            : '暂无数据'}
        </div>
      ) : (
        <div>
          {record.updateTableNameMessage && record.primaryKeyErrorMessage !== 'null'
            ? record.updateTableNameMessage
            : '暂无数据'}
        </div>
      )}
    </div>
