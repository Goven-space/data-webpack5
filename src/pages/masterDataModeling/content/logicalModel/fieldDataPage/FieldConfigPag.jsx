...
import { Row, TreeSelect, Button, Col, Transfer, Modal, Form, Drawer } from 'antd';
import MainContext from '@store';
import { standardManage } from '@api/dataAccessApi';
import { showInfo, getTreeSelectData, showError } from '@tool';
import CMForm from '@/components/cmForm';

const { getTypeOpt, getFieldOpt, saveConfig, showField, checkField, addCustom } =
...
export default function FieldConfigPag({ id, handleCancel, getTableData }) {
  ...
  const getInitData = () => {
    getTypeOpt({ classifyType: 'normDataElement' }).then(res => {
  ...
  const getAddFieldInitData = () => {
    //获取字段类型下拉
    getFieldTypeDataElementOpt({ type: localStorage.getItem('type') }).then(res => {
      const { state, data } = res.data;
      if (state) {
        const newData = [];
      ...
      if (state) {
        const newData = data.map(item => ({
          ...
          title: `${item.fieldName}-${item.fieldComment}-${
            item.metadataType === 'custom' ? '自定义' : '数据元'
          }`,
        }));
        setMockData(newData);
      }
  ...
  return (
    <div>
      <Drawer visible={visible} onClose={handleModalCancel} width={800} title="新增自定义字段">
        {<CMForm {...cmformConfig} />}
      </Drawer>
