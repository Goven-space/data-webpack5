...
const { getReferenceRuleField } = qualityManageApi.qualityRule;
...
export default function ListByRuleId(props) {
  ...
  const loadData = (pagination = { current, pageSize }) => {
    ...
    getReferenceRuleField(params).then(res => {
  ...
  const columns = [
    ...
    {
      title: '引用模型字段注释',
      dataIndex: 'dataModelFieldComment',
      width: '20%',
    },
    {
      title: '引用模型字段类型',
      dataIndex: 'dataModelFieldType',
      width: '10%',
    },
    {
      title: '引用模型字段长度',
      dataIndex: 'dataModelFieldLength',
      width: '10%',
    },
  ];
